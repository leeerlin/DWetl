#!/bin/bash
#check_imp_file_create.sh
#*=================================================
#*
#* FileName : check_imp_file_create.sh
#* CreateDate: 2016-03-3 
#* Abstract : 
#* 入参：模式名
#* 导出文件中提取信息，通过多导出的结构文件进行分析，将明细数据导出到临时文件$DW_HOME/file/tmp/import_file_create/{模式名}.lst
...
echo "" > ../file/tmp/import_file_create/${SCHEMA}.lst
LINE2=""
    ...
    if [[ ${LINE1} != "" ]] ; then
    	if [[ $LONE_LINE =~ "CREATE " || $LONE_LINE =~ "create " ]] ; then
    		if [[ $LONE_LINE =~ " PROCEDURE " ]] ; then
    			...
    			echo $INFO'=' >> ${PWDNOW}/../file/tmp/import_file_create/${SCHEMA}.lst
    		elif [[ $LONE_LINE =~ " procedure " ]] ; then
    			INFO=`echo $LONE_LINE | sed "s/create procedure/=PROCEDURE=/g" | sed "s/create or replace procedure/=PROCEDURE=/g" | sed "s/ //g" | sed "s/\"//g" | awk -F"(" '{print $1}' | sed "s/\./=/g"`
    			echo $INFO'=' >> ${PWDNOW}/../file/tmp/import_file_create/${SCHEMA}.lst
    		elif [[ $LONE_LINE =~ " TABLE " ]] ; then
    			INFO=`echo $LONE_LINE | sed "s/CREATE TABLE/=TABLE=/g" | sed "s/ //g" | sed "s/\"//g" | awk -F"(" '{print $1}' | sed "s/\./=/g"`
    			echo $INFO'=' >> ${PWDNOW}/../file/tmp/import_file_create/${SCHEMA}.lst
